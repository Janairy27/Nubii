...
export default function ReporteDiagnosticoProf() {
  ...
  const fetchReport = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:4000/api/repDiagnostico/info-diagnostico-profesional`,
        {
          params: {
            idProfesional,
            idPaciente,
  ...
  const chartData = useMemo(() => {
    ...
    const pacientesUnicos = [
      ...new Set(reportData.map((item) => item.nombrePaciente)),
    ];
    const tiposTestUnicos = [
      ...new Set(reportData.map((item) => item.tipo_test)),
    ];
      ...
      return {
        ...
        data: pacientesUnicos.map((pacientes) => {
          const item = reportData.find(
            (r) =>
              r.nombrePaciente === pacientes &&
              Number(r.tipo_test) === tipoInfo.value
          );
