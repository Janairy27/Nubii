...
const modalidadMap = [
  { value: 1, nombre: "Presencial", color: "#4CAF50", icon: <SchoolIcon fontSize="small" /> },
  { value: 2, nombre: "Virtual", color: "#2196F3", icon: <ComputerIcon fontSize="small" /> },
];
...
export default function ReporteCitaProf() {
  ...
  const fetchReport = async () => {
    ...
    setReportData([]);
    try {
      const res = await axios.get(`http://localhost:4000/api/repCita/info-citas`, {
        params: { idProfesional, idPaciente, fechaInicio, fechaFin, tipoReporte 
          
        }
      });
      if (res.data.length === 0) {
        mostrarMensaje("No hay información disponible para este profesional en el rango seleccionado.", "warning");
      }
              ...
              >
                ...
                >
                  ...
                  <Select
                    ...
                    label="Paciente"
                    onChange={(e) =>
                      setIdPaciente(e.target.value === "todos" ? null : Number(e.target.value))
                    }
                  >
                    <MenuItem value="todos">
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <GroupsIcon sx={{ color: "#64748b", fontSize: 20 }} />
                        <Typography fontWeight="500">Todos los pacientes</Typography>
                      </Box>
                    </MenuItem>
                    {pacientes.map((p) => (
            ...
            {reportData.length > 0 ? (
              <Card sx={{
                ...
                <Table sx={{ minWidth: 650, borderCollapse: "separate", borderSpacing: "0 10px" }}>
                  <TableHead>
                    <TableRow>
                      {["Paciente", "Fecha", "Duración", "Modalidad", "Estado", "Comentario"].map((head) => (
                  ...
                  <TableBody>
                    {reportData.map((row, i) => {
                      const estadoC = estadoCitaMap.find((m) => m.value === row.idEstado || m.value === row.estado);

                      const modalidad = modalidadMap.find((m) => m.value === row.idModalidad || m.value === row.modalidad);
                      ...
                      return (
                        <TableRow key={i} hover
                          ...
                          <TableCell>{format(new Date(row.fecha_cita), "yyyy-MM-dd")}</TableCell>
                          <TableCell>{row.duracion_horas} hrs</TableCell>
                          <TableCell>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                              <span style={{ color: modalidad?.color }}>{modalidad?.icon}</span>
                              <Typography>{modalidad?.nombre || row.modalidad}</Typography>
                            </Box>
                          </TableCell>
                          <TableCell>
                            <Box
