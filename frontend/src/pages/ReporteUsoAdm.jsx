...
export default function ReporteUso() {
 ...
 const [valoresFiltro, setValoresFiltro] = useState({
  ...
  const fetchReport = async () => {
    ...
    if (filtrosActivos.usuario && nombreUsuario) params.nombreUsuario = nombreUsuario;
    ...
    try {
      const res = await axios.get(`http://localhost:4000/api/repUso/info-uso`, { params }
      );
  ...
  const chartData = useMemo(() => {
    ...
    const actividades = reportData.map(item => {
      if (item.tipo_usuario === "Paciente") return Number(item.actividades_completadas || 0);
      if (item.tipo_usuario === "Profesional") return Number(item.actividades_publicadas || 0);
      return 0;
    });

    const tests = reportData.map(item => {
      if (item.tipo_usuario === "Paciente") return Number(item.total_test_realizados || 0);
      if (item.tipo_usuario === "Profesional") return Number(item.test_aplicados || 0);
      return 0;
    });
        ...
        <Paper sx={{
          ...
          <Box sx={{
            ...
            <Box ox sx={{
              ...
              > Reporte de uso del sistema</Typography>
            </Box>
              ...
              {reportData.length > 0 ? (
                  ...
                  <Card sx={{
                    ...
                    <Table sx={{
                      ...
                      <TableHead sx={{ backgroundColor: '#e8f0fe' }}>
                        <TableRow>
                          {["Nombre del usuario", "Tipo usuario", "No. test realizados",
                            "No. actividades completadas", "Promedio satisfacción",
                            "No. actividades publicadas", "Test aplicados", "Última actividad",
                            ].map((head) => (
                      ...
                      <TableBody>
                        {reportData.map((row, index) => (
                          <TableRow key={index} sx={{
                            ...
                            <TableCell align="center">{row.total_test_realizados}</TableCell>
                            <TableCell align="center">{row.actividades_completadas}</TableCell>
                            <TableCell align="center">{row.promedio_satisfaccion}</TableCell>
                            <TableCell align="center">{row.actividades_publicadas}</TableCell>
                            <TableCell align="center">{row.test_aplicados}</TableCell>
                            <TableCell align="center">{row.ultima_actividad}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                ...
                <Card sx={{ p: 3, textAlign: "center", borderRadius: 3 }}>
                  <Typography color="text.secondary">No hay datos para mostrar. Selecciona un rango o tipo de reporte.
                  </Typography>
                </Card>
              )}
            </Box>
