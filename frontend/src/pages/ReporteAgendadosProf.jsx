...
export default function ReporteAgendados() {
  ...
  const fetchReport = async () => {
    if (!idProfesional) return mostrarMensaje("No se ha encontrado el profesional.", "error");
    const params = { idProfesional };

    if (filtrosActivos.paciente && nombrePaciente) params.nombrePaciente = nombrePaciente;
    ...
    try {
      const res = await axios.get(`http://localhost:4000/api/repProfAgendados/info-agendas`, { params }
      );
        ...
        <Paper sx={{
          ...
          <Box sx={{
            ...
            <Box ox sx={{
              ...
              >
                Reporte de pacientes más agendados</Typography>
            </Box>
                ...
                <Card sx={{
                  ...
                  <Table sx={{
                    ...
                    <TableHead sx={{ backgroundColor: '#e8f0fe' }}>
                      <TableRow>
                        {["Nombre del paciente", "No. citas", "No. citas concluidas",
                          "No. citas fallidas", "No. citas aceptadas",
                          "No. citas pendientes", "Primera cita", "Última cita",
                        ].map((head) => (
                    ...
                    <TableBody>
                      {reportData.map((row, index) => (
                        <TableRow key={index} sx={{
                          ...
                          <TableCell align="center"
                            sx={{ color: '#f3db21ff', fontWeight: 'bold' }}>{row.citas_pendientes}</TableCell>
