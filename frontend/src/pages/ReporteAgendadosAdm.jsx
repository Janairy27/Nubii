...
const getEspecialidadDetails = (id) => {
  return (
    especialidadMap.find((e) => e.value === id) || {
      nombre: "Desconocida",
      icono: null,
      color: "#6c757d",
    }
...
export default function ReporteProfAgendados() {
  ...
  const fetchReport = async () => {
    ...
    if (filtrosActivos.usuario && nombreProfesional)
      params.nombreProfesional = nombreProfesional;
    if (filtrosActivos.fecha && fechaInicio && fechaFin) {
      params.fechaInicio = fechaInicio;
      params.fechaFin = fechaFin;
    }
    if (filtrosActivos.especialidad && especialidad !== "")
    ...
    try {
      const res = await axios.get(
        `http://localhost:4000/api/repProfAgendados/info-profesionales-agendados`,
        { params }
      );
  ...
  const chartData = useMemo(() => {
    ...
    const backgroundColors = reportData.map(
      (item) => getEspecialidadDetails(item.id_especialidad).color
    );
    const borderColors = reportData.map(
      (item) => getEspecialidadDetails(item.id_especialidad).color
    );
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                Reporte de pofesionales más agendados
              </Typography>
            </Box>
            {/* Filtros */}
            <Card
                  ...
                  >
                    ...
                    <TableBody>
                      {reportData.map((row, index) => {
                        const espDetails = getEspecialidadDetails(
                          row.id_especialidad
                        );
                            ...
                            <TableCell align="center">
                              <Box
                                sx={{
                                  ...
                                  justifyContent: "center",
                                  gap: 1,
                                  color: espDetails.color,
                                  fontWeight: "bold",
                                }}
                              >
                                {espDetails.icono &&
                                  React.cloneElement(espDetails.icono, {
                                    sx: { color: espDetails.color },
                                  })}
                                {espDetails.nombre}
                              </Box>
                            </TableCell>
                          ...
                          tooltip: {
                            ...
                            callbacks: {
                              ...
                              afterLabel: (context) => {
                                ...
                                const espName =
                                  getEspecialidadDetails(espId).nombre;
                                return [
