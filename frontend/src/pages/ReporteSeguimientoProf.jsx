...
export default function ReporteSeguimientoProf() {
  ...
  const resultadoMap = [
    ...
    { value: 2, nombre: "Leve", icono: <SentimentSatisfiedIcon />, color: "#9ccc65", },
  ...
  const fetchReport = async () => {
    if (!idProfesional) return mostrarMensaje("No se ha encontrado el profesional.");
    setReportData([]);
    ...
    try {
      const res = await axios.get(`http://localhost:4000/api/repSeguimiento/info-seguimiento-pacientes`, { params }
      );
  ...
  const exportPDF = async () => {
    try {
      if (reportData.length === 0) return mostrarMensaje("Primero genera el reporte.");
  ...
  const chartData = useMemo(() => {
    ...
    const categorias = [...new Set(reportData.map(item => item.categoria_test))];

    const promedioIntensidad = categorias.map(cat => {
      const items = reportData.filter(item => item.categoria_test === cat);
      const sum = items.reduce((acc, cur) => acc + Number(cur.promedio_intensidad_emocional), 0);
      ...
      datasets: [
        {
          ...
          backgroundColor: categorias.map(cat => testMap[cat]?.color || '#42A5F5'),
        },
        {
          label: 'Último Test (promedio)',
          data: ultimoTest,
          backgroundColor: categorias.map(cat => testMap[cat]?.color ? `${testMap[cat].color}B0` : '#FFA726B0'),
        }
      ]
    };
  }, [reportData, testMap]);
        ...
        <Paper sx={{
          ...
          <Box sx={{
            ...
            <Box sx={{ textAlign: "center", mb: 4 }}>
              <Typography variant="subtitle1"><strong>Profesional:</strong> {Nombre || "Cargando..."}</Typography>
                    ...
                    >
                      {Object.entries(fullEmocionMap).map(([id, emo]) => (
                        <MenuItem key={id} value={id}>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1, color: emo.color }}>
                            {React.createElement(emo.icon, { sx: { fontSize: 20 } })}
                            {emo.label}
                          </Box>
                        </MenuItem>
                      ))}
                    ...
                    >
                      ...
                      {TipoTest.map((test) => (
                        <MenuItem key={test.value} value={test.nombre}>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1, color: test.color }}>
                            {React.createElement(test.icono, { sx: { fontSize: 20 } })}
                            {test.nombre}
                          </Box>
                        </MenuItem>
                      ))}
                ...
                <Card sx={{
                  ...
                  <Table sx={{
                    ...
                    <TableHead sx={{ backgroundColor: '#e8f0fe' }}>
                      <TableRow>
                        {["Paciente", "Fecha último test", "Total síntomas",
                          "Promedio intensidad", "Emoción predominante",
                          "Promedio historial", "Nivel dominante", "Último test",
                          "Puntaje test", "Categoría test"].map((head) => (
                    ...
                    <TableBody>
                      {reportData.map((row, index) => {
                        ...
                        const ultimoT = TipoTest.find((m) => m.value === idTest);

                        const categoriaTes = resultadoMap.find((m) => m.value === row.idCategResul || m.value === row.categoria_test);
                          ...
                          <TableRow key={index} sx={{
                            ...
                            <TableCell align="center">
                              {row.fecha_ultimo_test
                                ? format(new Date(row.fecha_ultimo_test), 'yyyy-MM-dd')
                                : '—'}
                            </TableCell>
                            <TableCell align="center">{row.total_sintomas}</TableCell>
                            <TableCell align="center">{row.promedio_intensidad_emocional}</TableCell>
                            <TableCell>
                              <Box
                                sx={{
                                  display: "flex",
                              ...
                              >
                                {emo?.icon && React.createElement(emo.icon, { sx: { fontSize: 20 } })}
                                {emo?.label || row.emocion_predominante}
                              </Box>
                            </TableCell>
                            <TableCell align="center">{row.intensidad_prom_historial}</TableCell>
                            <TableCell>
                              <Box
                                sx={{
                                  display: "flex",
                              ...
                              >
                                {categoriaTes?.icono}
                                {categoriaTes?.nombre || row.nivel_dominante_historial}
                            ...
                            <TableCell align="center">{row.puntaje_test}</TableCell>
