...
export default function ReporteDiagnosticoPac() {
  ...
  const fetchReport = async () => {
    ...
    setReportData([]);
    try {
      const res = await axios.get(`http://localhost:4000/api/repDiagnostico/info-DiagnosticoPac`, {
        params: {
          idPaciente,
          fechaInicio,
          fechaFin,
      ...
      if (res.data.length === 0) {
        mostrarMensaje(" No hay información disponible len el rango seleccionado.", "warning");
      }
  ...
  const chartData = useMemo(() => {
    ...
    const uniqueTestIds = [...new Set(reportData.map(item => Number(item.tipo_test)))];
    ...
    const datasets = uniqueTestIds.map(testId => {
      ...
      const dataForTest = reportData.filter(item => Number(item.tipo_test) === testId);
      return {
        label: testName,
        backgroundColor: testColor,
        borderWidth: 1,
        data: dataForTest.map(item => Number(item.promedio_puntaje).toFixed(2)),
      };
    });
                  ...
                  <Table sx={{
                    ...
                    <TableBody>
                      {reportData.map((row, index) => {
                        ...
                        const resultInfo = resultadoMap.find((r) => r.value === idResultado);
                        ...
                        return (
                          <TableRow key={index}
                            ...
                            <TableCell align="center" sx={{ color: "#555" }}>{row.total_aplicaciones}</TableCell>
                            <TableCell align="center" sx={{ fontWeight: 700, color: "#1976d2" }}>{Number(row.promedio_puntaje).toFixed(2)}</TableCell>
                            <TableCell align="center" sx={{ color: "#555", fontSize: '0.85rem' }}>{format(new Date(row.ultima_fecha), "yyyy-MM-dd")}</TableCell>
                            <TableCell sx={{ fontWeight: 600 }}>
                              {/* Celda de Categoría: Muestra Icono y Color */}
                              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                <Box sx={{ color: resultInfo?.color || "gray" }}>
                                ...
                                >
                                  {resultInfo?.nombre || row.categorias_resultado}
                                </Box>
                              </Box>
                            </TableCell>
                            <TableCell align="center">{row.interpretaciones || "-"}</TableCell>
                            <TableCell align="center">{row.recomendaciones || "-"}</TableCell>
                          </TableRow>
                        );
                      })}
                    </TableBody>
