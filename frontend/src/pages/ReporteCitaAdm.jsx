...
export default function ReporteCitaAdm() {
  ...
  const fetchReport = async () => {
    setReportData([]);
    try {
      const res = await axios.get(`http://localhost:4000/api/repCita/info-citas-profesionales`);

      if (res.data.length === 0) {
        mostrarMensaje(" No hay información disponible para este rango seleccionado.", "warning");
      }
      ...
      {reportData.map((row, index) => {
        const especialidad =
          especialidadMap.find(
            (m) => m.value === row.idEspecialidad || m.value === row.especialidad
          ) || {};
        ...
        return (
          ...
          >
            <TableCell sx={{ fontWeight: 500 }}>{row.nombreProfesional}</TableCell>
            <TableCell sx={{ color: "#546e7a" }}>{row.email}</TableCell>
            ...
            <TableCell>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                />
                <span style={{ color: especialidad?.color }}>{especialidad?.icono}</span>
