...
export const findActividadById = async (
  idActividad,
  idProfesional,
  publico
) => {
  const [rows] = await db.query(
    "SELECT * FROM Actividad WHERE idActividad = ? AND idProfesional = ? AND publico = ?",
    [idActividad, idProfesional, publico]
  );
  return rows[0];
};
...
export const findActividadByActividad = async (idActividad) => {
  const [rows] = await db.query(
    `SELECT nombreAct, fechaPublicado FROM Actividad WHERE idActividad = ? AND publico = 2`,
    [idActividad]
  );
  return rows[0];
};
...
export const getActividadesByProfesionalDesc = async (
  idProfesional,
  publico
) => {
  const [rows] = await db.query(
    "SELECT * FROM Actividad WHERE idProfesional = ? AND publico = ? ORDER BY idActividad DESC",
...
export const findActividadByAttributeProfesional = async (
  atributo,
  valor,
  idProfesional,
  publico
...
export const getActividadesByAttributeUsuarios = async (
  filtros = {},
  idPaciente
) => {
  let query = `
