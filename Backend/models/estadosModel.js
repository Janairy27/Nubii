...
export const obtenerEstadoById = async(idCita) => {
    const [rows] = await db.query ("SELECT * Estado_cita WHERE idCita = ?", [idCita]);
    return rows[0];
};

export const obtenerEstadoFinal = async(idCita) => {
    const [rows] = await db.query("SELECT estado FROM Estado_cita WHERE idCita = ? ORDER BY fecha_cambio DESC LIMIT 1", [idCita]);
    return rows[0];
};