...
export const deleteResultados = async (idResultadoTest) => {
  ...
  const [result] = await db.query(
    "DELETE FROM ResultadoTest WHERE idResultadoTest = ?",
    [idResultadoTest]
  );
  ...
  if (filasAfectadas === 0) {
    ...
    throw new Error(
      `No se encontró el resultado de test con ID ${idResultadoTest} para eliminar.`
    );
  }
...
export const getResultadoByAtrributeProf = async (
  filtros = {},
  idProfesional
) => {
  let query = `
...
export const getResultadoByAtrributePac = async (filtros = {}, idPaciente) => {
  ...
  if (filtros.nombreProfesional) {
    query +=
      "AND CONCAT(pro.Nombre, ' ', pro.aPaterno, ' ', pro.aMaterno) LIKE ? ";
...
export const getAllResultadosTestByAttribute = async (filtros = {}) => {
  ...
  if (filtros.nombrePaciente) {
    query +=
      "AND CONCAT(u.Nombre, ' ', u.aPaterno, ' ', IFNULL(u.aMaterno, ' ')) LIKE ? ";
    parametros.push(`%${filtros.nombrePaciente}%`);
  }

  if (filtros.nombreProfesional) {
    query +=
      "AND CONCAT(pr_u.Nombre, ' ', pr_u.aPaterno, ' ', IFNULL(pr_u.aMaterno, ' ')) LIKE ? ";
    parametros.push(`%${filtros.nombreProfesional}%`);
  }
  ...
  try {
    ...
    console.error(
      "Error al consultar todos los resultados de test (Admin):",
      err
    );
    throw err;
  }
