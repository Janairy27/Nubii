...
export const updateCita = async (idCita, fecha, CitaData) => {
  ...
  const [result] = await db.query(
    `UPDATE Cita SET
        ...
        WHERE idCita = ?`,
    [fecha, CitaData.modalidad, CitaData.enviado, idCita]
  );
...
export const updateCitaProf = async (idCita, CitaData) => {
  const [result] = await db.query(
    `UPDATE Cita SET
        ...
        WHERE idCita = ?`,
    [CitaData.duracion_horas, CitaData.enlace, CitaData.comentario, idCita]
  );
  console.log("Cita actualizada");
  return result.affectedRows;
};
...
export const getInfoCita = async (idCita) => {
  const [rows] = await db.query("SELECT * FROM Cita WHERE idCita = ?", [
    idCita,
  ]);
  return rows[0];
};
...
export const getCitasByAtrributeAdmin = async (filtros = {}) => {
  ...
  if (filtros.nombrePaciente) {
    query +=
      "AND CONCAT(up.Nombre, ' ', up.aPaterno, ' ', up.aMaterno) LIKE ? ";
    parametros.push(`%${filtros.nombrePaciente}%`);
  }
  ...
  if (filtros.nombreProfesional) {
    query +=
      "AND CONCAT(upr.Nombre, ' ', upr.aPaterno, ' ', upr.aMaterno) LIKE ? ";
    parametros.push(`%${filtros.nombreProfesional}%`);
  }
  ...
  try {
    ...
    console.log(
      "Error al consultar resultados de citas para administrador:",
      err
    );
    throw err;
  }
