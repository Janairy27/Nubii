...
export const reporteProfesionalesAdm = async(filtros = {}) => {
    ...
    if (filtros.nombreProfesional) {
        query += " AND CONCAT(prof.Nombre, ' ', prof.aPaterno, ' ', prof.aMaterno) LIKE ? ";
    ...
    try{
        ...
        console.log("Error al obtener reporte de profesionales más agendados:", err);
        throw err;
    }
};
...
export const reporteAgendadosProf = async(filtros = {}, idProfesional) => {
    ...
    if (filtros.nombrePaciente) {
        query += " AND CONCAT(pa.Nombre, ' ', pa.aPaterno, ' ', pa.aMaterno) LIKE ? ";
