...
export const modifyRecomendacion = async (
  idRecomendacion,
  RecomendacionData
) => {
  const [result] = await db.query(
...
export const modifyRecomendacionPaciente = async (
  idRecomendacion,
  RecomendacionData
) => {
  console.log(
    `[DB] Actualizando ID ${idRecomendacion} con mejoramiento: ${RecomendacionData.mejoramiento}`
  );
  const [result] = await db.query(
    `
        UPDATE Recomendacion 
...
export const getActividadById = async (idRecomendacion) => {
  const [rows] = await db.query(
    "SELECT * FROM Recomendacion WHERE idRecomendacion = ?",
    [idRecomendacion]
  );
  return rows[0];
};
