...
export const changeDate = async (fecha, idUsuario) => {
  await db.query("UPDATE Usuario set activo = ? WHERE idUsuario = ?", [
    fecha,
    idUsuario,
  ]);
};
...
export const updatePassword = async (idUsuario, hashedPassword) => {
  await db.query(
    "UPDATE Usuario SET contrasena = ?, reset_token = NULL, reset_token_expires = NULL WHERE idUsuario = ?",
...
export const getFullName = async (idUsuario) => {
  const [rows] = await db.query(
    "SELECT Nombre, aPaterno, Amaterno FROM Usuario WHERE idUsuario = ?",
    [idUsuario]
  );
  return rows[0];
};
