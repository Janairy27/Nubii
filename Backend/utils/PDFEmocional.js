  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor('#FFFFFF')
        .font("Helvetica-Bold")
        .fontSize(18)
        .text("Reporte emocional del paciente", 0, headerY - 5, { align: "center" });
      ...
      doc.fontSize(11)
        .font("Helvetica")
        .text(`Paciente: ${nombrePaciente || 'No especificado'}`, 0, headerY + 25, { align: "center" });

      doc.text(`Tipo de Reporte: ${tipoReporte || 'General'} | Fecha: ${new Date().toLocaleDateString('es-ES')}`, 
      ...
      const headers = ["Paciente", "Fecha", "Emoción", "Intensidad", "Registros"];
      ...
      datos.forEach((row, index) => {
        ...
        const cantidad = row.cantidad || row.cantidad_registros || row.total_registros || 0;
        ...
        // Columna Emoción 
        doc.fillColor(emocionData.color)
          .font("Helvetica-Bold")
          .text(emocionData.nombre, startX + columnWidths.slice(0, 2).reduce((a, b) => a + b, 0) + 5, y + textYOffset, {
        ...
        doc.fillColor(colorIntensidad)
          .text(promedio, startX + columnWidths.slice(0, 3).reduce((a, b) => a + b, 0) + 5, y + textYOffset, {
        ...
        // Columna Registros
        doc.fillColor(COLORES.texto)
          .text(cantidad.toString(), startX + columnWidths.slice(0, 4).reduce((a, b) => a + b, 0) + 5, y + textYOffset, {
...
export async function PDFProfesional(datos, graficoBase64, nombreReporte = "Reporte de Gestión Emocional", tipoReporte = "Agregado", ) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ 
        margin: 40, 
      ...
      //  Logo en la esquina superior izquierda
      if (typeof fs !== 'undefined' && LOGO_FILE_PATH && fs.existsSync(LOGO_FILE_PATH)) {
        doc.image(LOGO_FILE_PATH, 50, headerY - 10, { fit: [40, 40] });
      }
      //  Título
      doc.fillColor('#FFFFFF')
      ...
      doc.fontSize(11)
        .font("Helvetica")
        .text(`Alcance: ${tipoReporte} (Múltiples Pacientes)`, 0, headerY + 25, { align: "center" });
      ...
      if (graficoBase64) {
        try {
          ...
          doc.font("Helvetica-Bold")
            .fontSize(12)
            .fillColor(COLORES.texto)
            .text("Evolución de intensidad emocional Agregada", { align: "center" });
      ...
      const tableHeaders = ["Paciente", "Período", "Emoción", "Intensidad", "Registros"];
