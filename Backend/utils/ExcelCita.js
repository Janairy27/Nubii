...
export async function generarExcel(datos, nombreHoja = "Reporte de Datos") {
    ...
    if (!Array.isArray(datos) || datos.length === 0) {
        throw new Error(" La entrada debe ser un array con datos para generar el reporte.");
    }
    ...
    const columnsConfig = keys.map((key, index) => {
        let headerText = key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, ' $1');
        let width = 25; 
    ...
    datos.forEach((rowData) => {
        ...
        const rowValues = columnsConfig.map((col) => {
            ...
            else if (key === fechaColumnKey && typeof value === 'string' && value.includes('T')) {
        ...
        if (rowData.hasOwnProperty(estadoCitaColumnKey) && keyToColId[estadoCitaColumnKey]) {
            ...
            if (color && cell) {
                ...
                cell.font = {
                    bold: true,
                    color: { argb: (['21D127', '66BC05', '81D4FA'].includes(cleanColor)) ? 'FF000000' : 'FFFFFFFF' },
                };
        ...
        if (rowData.hasOwnProperty(especialidadColumnKey) && keyToColId[especialidadColumnKey]) {
        ...
        if (rowData.hasOwnProperty(modalidadColumnKey) && keyToColId[modalidadColumnKey]) {
