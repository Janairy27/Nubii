import {
  reporteSeguimientoPac,
  reporteSeguimientoProf,
} from "../models/repSeguimientoModel.js";
...
export const infoSeguimientoPaciente = async (req, res) => {
  try {
    ...
    const informacion = await reporteSeguimientoPac(
      filtros,
      parseInt(idPaciente)
    );
  ...
  try {
    ...
    const filtros = {
      ...
      emocionPredominante: emocionPredominante
        ? parseInt(emocionPredominante)
        : undefined,
      intensidadMinima: intensidadMinima
        ? parseInt(intensidadMinima)
        : undefined,
      intensidadMaxima: intensidadMaxima
        ? parseInt(intensidadMaxima)
        : undefined,
      tipoTest: tipoTest || undefined,
      tipoReporte: tipoReporte || undefined,
    };

    const informacion = await reporteSeguimientoProf(
      filtros,
...
export const exportarPDF = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="ReporteSeguimiento.pdf'
    );
    res.send(pdf);
  } catch (err) {
    console.log(err);
