import {
  createActividad,
  getInfoPacienteyProfesional,
  updateActividad,
  deleteActividad,
  findActividadById,
  findActividadByActividad,
  findActividadByAttributeProfesional,
  getActividadesByProfesionalDesc,
  getActividadesByAttributeUsuarios,
  getActividadById,
...
export const registrarActividad = async (req, res) => {
  ...
  const { nombreAct, descripcionAct, objetivo, fechaPublicado } = req.body;
  ...
  try {
    ...
    if (!tipoAct)
      return res
        ...
        .json({
          message:
            "La actividad es obligatoria, debes de seeleccionar un botón",
        });
    if (!duracion_minutos)
      return res.status(400).json({ message: "La duración es obligatoria" });
    if (!dificultad)
      return res
        .status(400)
        .json({
          message:
            "La dificultad es obligatoria, debes de seleccionar un botón ",
        });
    if (!objetivo)
    ...
    if (!publico)
      return res
        ...
        .json({
          message:
            "La publicación es obligatoria, debes de seleccionar un botón",
        });
      ...
      if (publico === 2) {
        ...
        for (const paciente of pacientes) {
          ...
          try {
            ...
            console.error(
              ` Error al enviar correo a ${emailPaciente}:`,
              sendError.message
...
export const ActualizarActividad = async (req, res) => {
  ...
  const { nombreAct, descripcionAct, objetivo } = req.body;
  ...
  console.log(
    `Duración: ${duracion_minutos} (es NaN: ${isNaN(duracion_minutos)})`
  );
  ...
  const multimedia = req.file ? req.file.filename : actividadActual.multimedia;
  ...
  try {
    ...
    if (!Actividad) {
      return res
        .status(404)
        .json({ message: "La actividad no fue actualizada" });
    }
...
export const getActividadesByProfesional = async (req, res) => {
  ...
  console.log(
    `Id del profesional obtenido para las actividades ${idProfesional} y publico ${publico}`
  );

  try {
    const actividad = await getActividadesByProfesionalDesc(
      idProfesional,
      publico
    );
...
export const getActividadesByAttributeProfesional = async (req, res) => {
  ...
  try {
    ...
    const resultado = await findActividadByAttributeProfesional(
      consulta,
      valor,
      idProfesional,
      publico
...
export const getActividadesByFilter = async (req, res) => {
  try {
    ...
    const resultado = await getActividadesByAttributeUsuarios(
      filtros,
      parseInt(idPaciente)
    );
    res.json(resultado);
