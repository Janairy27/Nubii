import { 
    createHistorial, getHistoriaByPaciente, getSintomasByPaciente, getHistorial
...
export const registrarHistorial = async(req, res) => {
    ...
    try{
        ...
        if(!fecha_inicio) return res.status(404).json({message: "Fecha de inicio obligatoria"});
        if(!fecha_fin) return res.status(404).json({message: "Fecha de fin obligatoria"});
        if(!emocion_predom) return res.status(404).json({message: "Emoción obligatoria"});
        if(!prom_intensidad) return res.status(404).json({message: "Promedio de intensidad obligatorio"});
        ...
        const total_dias = (new Date(fecha_fin) - new Date(fecha_inicio)) / (1000 * 60 * 60 * 24);
...
export const obtenerInfo = async(req, res) => {
    ...
    try{
        ...
        if(!historial) return res.status(404).json({message: "Usuario no encontrado"});
        res.json(historial);
    }catch(err){
...
export const obtenerSintoma = async(req, res) => {
    ...
    try{
        ...
        if(!sintoma) return res.status(404).json({message: "Usuario no encontrado"});
        res.json(sintoma);
    }catch(err){
        res.status(500).json({error: err.message});
    }
