...
import { obtenerPacientesPorProfesional, updateMensajesLeidos,
   obtenerMensajesNoLeidosModel, obtenerMensajesNoLeidosPacModel } from "../models/chatModel.js";
...
export const obtenerChat = async (req, res) => {
  ...
  if (isNaN(idPaciente) || isNaN(idProfesional)) {
    return res.status(400).json({ message: "ID paciente o profesional inválido" });
  }
...
export const marcarMensajesLeidos = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: "Error al marcar mensajes como leídos" });
  }
};
...
export const obtenerMensajesNoLeidos = async (req, res) => {
  ...
  if (!idProfesionalParam) {
    ...
    return res.status(400).json({ message: "ID paciente o profesional inválido (Faltante)" });
  }
  ...
  if (isNaN(idProfesional) || idProfesional <= 0) {
    ...
    console.error(`Fallo de validación: El ID parseado (${idProfesional}) no es un número entero válido.`);
    return res.status(400).json({ message: "ID paciente o profesional inválido" });
  }
...
export const obtenerMensajesNoLeidosPac = async (req, res) => {
  ...
  if (isNaN(idPaciente) || idPaciente <= 0) {
    ...
    console.error(`Fallo de validación: El ID parseado (${idPaciente}) no es un número entero válido.`);
    return res.status(400).json({ message: "ID paciente o profesional inválido" });
  }
