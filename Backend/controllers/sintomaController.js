import {
  ...
  deleteSintoma,
  findSintomaById,
  getSintomasByPacienteDesc,
  findSintomaByAttribute,
...
export const registrarSintoma = async (req, res) => {
  ...
  const { fecha, detonante, ubicacion, actividadReciente, nota } = req.body;
  ...
  try {
    ...
    if (!emocion) {
      return res
        .status(400)
        .json({ message: "Debes de seleccionar una emoción" });
    }
    if (!intensidad) {
      return res
        .status(400)
        .json({ message: "Debes de seleccionar una intensidad" });
    }
    if (!clima) {
    ...
    if (errores.length > 0) {
      ...
      return res
        .status(400)
        .json({ message: "Cumplir con el formato solicitado", errores });
    }
...
export const ActualizarSintoma = async (req, res) => {
  ...
  try {
    ...
    if (errores.length > 0) {
      return res
        .status(400)
        .json({ message: "Errores de validacion", errores });
    }

    console.log(`Actualizando síntoma con id ${idSintoma}`);
    const actualizando = await updateSintoma(idSintoma, {
      fecha,
      emocion,
      intensidad,
      clima,
...
export const getSintomasByAttributeProf = async (req, res) => {
  try {
    const { nombrePaciente, nivel_estres, fecha, emocion, intensidad, clima } =
      req.query;
...
export const getSintomasByAttributePaciente = async (req, res) => {
  ...
  try {
    ...
    const resultado = await findSintomaByAttributePaciente(
      consulta,
      valor,
      idPaciente
    );
