...
import {
  createPaciente,
  updatePaciente,
  deleteRelacionPaciente,
} from "../models/pacienteModel.js";
import {
  createProfesional,
  updateProfesional,
  deleteRelacionProfesional,
...
export const register = async (req, res) => {
  ...
  try {
    ...
    if (!contrasena)
      return res.status(400).json({ message: "Falta ingresar contraseña" });
    if (!sexo)
      return res.status(400).json({ message: "Falta seleccionar sexo" });
    ...
    if (!tipo_usuario)
      return res
        .status(400)
        .json({ message: "Faltan seleccionar un tipo de usuario" });
    ...
    if (tipo_usuario === 2) {
      if (!especialidad)
        return res
          .status(400)
          .json({ message: "La especialidad es obligatoria" });
    }
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Usuario registrado con éxito", id_usuario });
    } catch (err) {
...
export const googleAuth = async (req, res) => {
  try {
    ...
    if (!id_token)
      return res.status(400).json({ message: "Falta de id_token" });
    ...
    // Envio de respuesta
    res.json({
      message: "Login con Google exisoto",
      token,
      role: user.tipo_usuario,
      name: user.nombre,
      email: user.email,
...
export const getUsuariosByFilter = async (req, res) => {
  try {
    const { nombreUsuario, email, estado, tipo_usuario, especialidad } =
      req.query;
