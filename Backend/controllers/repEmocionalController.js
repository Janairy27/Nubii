import { reporteEmocional, reporteEmocionalProf } from "../models/RepEmocionalModel.js";
...
export const infoEmocionalPaciente = async(req, res) => {
    try{
        ...
        if (isNaN(idPacienteNum) || idPacienteNum <= 0) {
            return res.status(400).json({ error: "El ID del paciente es inválido (debe ser un número entero positivo)." });
        }
...
export const infoEmocionalPorProfesional = async(req, res) => {
    try{
        const{idProfesional, idPaciente, nombrePaciente, fechaInicio, fechaFin, tipoReporte } = req.query;
        console.log("Parametros recibidos:", req.query);
        ...
        const informacion = await reporteEmocionalProf(filtros, parseInt(idProfesional));
        return res.json(informacion);
    }catch(err){
        console.log("Error al obtener información del reporte:", err);
        return res.status(500).json({error: err.message});
...
export const exportarExcel = async(req, res) => {
    try{
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="ReporteEmocional.xlsx');
   ...
   // console.log("Parámetros enviados:", {  fechaInicio, fechaFin, tipoReporte });
    ...
    try{
       ...
       // console.log("Parámetros recibidos:", req.query);
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="ReporteEmocional.pdf"');
        res.send(pdf);
    }catch(err){
        console.log(err);
