import { reporteProfesionalesAdm, reporteAgendadosProf } from "../models/repProfAgendadosModel.js";
...
export const infoCitasAgAdmin = async(req, res) => {
    try{
        const{nombreProfesional, especialidad, citasMin, citasMax, fechaInicio, fechaFin, tipoReporte } = req.query;
...
export const infoAgendadosProf = async(req, res) => {
    try{
        const{idProfesional, nombrePaciente, fechaInicio, fechaFin, tipoReporte } = req.query;
        console.log("Parametros recibidos:", req.query);
        ...
        const informacion = await reporteAgendadosProf(filtros, parseInt(idProfesional));
        return res.json(informacion);
    }catch(err){
        console.log("Error al obtener información del reporte:", err);
        return res.status(500).json({error: err.message});
...
export const exportarExcel = async(req, res) => {
    try{
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="ReporteProfesionalesAgendados.xlsx');
