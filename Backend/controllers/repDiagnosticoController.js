import { reporteDiagnosticoPac, reporteDiagnosticoProf } from "../models/repDiagnosticosModel.js";
import { generarPDF, PDFProfesional } from "../utils/PDFDiagnostico.js";

export const infoDiagnosticoPaciente = async(req, res) => {
    try{
        const{idPaciente, fechaInicio, fechaFin, tipoReporte, puntajeMin, puntajeMax } = req.query;
        ...
        const informacion = await reporteDiagnosticoPac(filtros, parseInt(idPaciente));
...
export const infoDiagnosticoPorProfesional = async(req, res) => {
    try{
        const{idProfesional, idPaciente, nombrePaciente, fechaInicio, fechaFin, tipoReporte, puntajeMin, puntajeMax } = req.query;
        console.log("Parametros recibidos:", req.query);
        ...
        const informacion = await reporteDiagnosticoProf(filtros, parseInt(idProfesional));
        return res.json(informacion);
    }catch(err){
        console.log("Error al obtener información del reporte:", err);
        return res.status(500).json({error: err.message});
...
export const exportarPDF = async(req, res) => {
    try{
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="ReporteDiagnostico.pdf');
        res.send(pdf);
    }catch(err){
        console.log(err);
