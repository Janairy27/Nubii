import {
  createRecomendacion,
  modifyRecomendacion,
  modifyRecomendacionPaciente,
  getActividadById,
  obtenerRecomendaciones,
  obtenerRecomendacionesPac,
  getRecomendacionById,
  getPacienteByRecomendacion,
...
export const registrarRecomendacion = async (req, res) => {
  ...
  try {
    ...
    if (!actividad)
      return res
        .status(404)
        .json({ message: "El nombre de la actividad es obligatoria" });
    if (!descripcion)
      return res.status(404).json({ message: "La descripción es obligatoria" });
    if (!idPaciente)
      return res
        .status(404)
        .json({ message: "El nombre del paciente es obligatorio" });
...
export const modificarRecomendacionProf = async (req, res) => {
  ...
  try {
    ...
    const Recomendacion = await modifyRecomendacion(idRecomendacion, {
      actividad,
      descripcion,
      publico,
    });
    if (!Recomendacion) {
      return res
        .status(404)
        .json({ message: "La recomendación no fue actualizada" });
    }
...
export const modificarRecomendacionPac = async (req, res) => {
  ...
  try {
    if (mejoramiento == null || !mejoramiento)
      return res
        .status(400)
        .json({ message: "La intensidad de mejora es obligatoria" });
    if (mas_recomendacion == null || !mas_recomendacion)
      return res
        .status(400)
        .json({
          message: "Es obligatorio indicar si se requieren más recomendaciones",
        });
    ...
    if (Recomendacion === 0) {
      return res.status(404).json({
        message:
          "La recomendación no fue modificada. El ID de recomendación no se encontró en la base de datos.",
      });
    }

    const pacienteInfo = await getPacienteByRecomendacion(idRecomendacion);
    const nombrePaciente =
      pacienteInfo?.nombrePaciente || "Paciente desconocido";
    ...
    if (idUsuario) {
      ...
      ${
        mejoramiento === 1
          ? "Mencionó que la actividad no ayudó mucho."
          : mejoramiento === 2
          ? "Indicó que tuvo una mejora moderada."
          : mejoramiento === 3
      ...
      ${
        mas_recomendacion === 2
          ? "También indicó que desea recibir más recomendaciones."
          : ""
      }
      `.trim();
