import { createChatMessage, getChatMessages, getPacienteByChat, obtenerMensajeCorreo } from "../models/chatModel.js";
...
export const handleNewMessage = async ({ idPaciente, idProfesional, mensaje, enviado }) => {
    
    const idChat = await createChatMessage({ idPaciente, idProfesional, mensaje, enviado });
    const fullMessage = { idMensaje: idChat, idPaciente, idProfesional, mensaje, enviado };
    
    let idReceptorRol = null; 
    let titulo = "";
    let mensajeNotif = "";

    // 1. Determinar el RECEPTOR y construir la notificación
    if (enviado === "paciente") {
    ...
    if (!idReceptorRol) {
        console.error("Error: ID de rol del receptor no pudo ser determinado o es nulo.");
        return fullMessage;
    }
    
    // ----------------------------------------------------
    //  Obtener la información del receptor (idUsuario, email, estaActivo)
    // ----------------------------------------------------
    let receptorInfo = null;

    //  Llamada simplificada y segura
    ...
    // Verificar si el usuario fue encontrado (receptorInfo no es null)
    if (!receptorInfo) {
    ...
    if (idUsuarioReceptor) {
        //  Crear notificación en la BD y sistema (siempre)
        ...
        if (!receptorInfo.estaActivo && receptorInfo.email) {
            console.log(`✉️ Enviando correo a ${receptorInfo.email}. Usuario inactivo.`);
